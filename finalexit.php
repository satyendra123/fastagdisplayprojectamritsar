...
function update_db_data($data) {
    $curl = curl_init();
    curl_setopt_array($curl, array(
    CURLOPT_URL => 'http://127.0.0.1:5000/update-database',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['CONTENT_TYPE']) && $_SERVER['CONTENT_TYPE'] === 'application/json') {
    ...
    if (isset($data['tagid']) && isset($data['token']) && isset($data['entryts']) && isset($data['signvalue']) && isset($data['tid']) && isset($data['vehtype']) && isset($data['tarrifvehtype']) && isset($data['avcvalue'])) {
        $token = $data['token'];
        if ($token == "SE9VU1lTLVBBUksyMDIzUFRN" || $token == "SE9VU1lTLVBBUksyMDIzUFRN1") {
            $db = getDb();
            ...
            if ($vehicleCategory != '') {
                ...
                if (!empty($tagInfoResp)) {
                    if ($tariffAmount < 1500) {
                       ...
                       update_db_data($python_data);
                    }
                else {
                    $exitRespCode = 300;
                    $response['msg'] = "Tariff amount exceeds limit";
                }
            } 
